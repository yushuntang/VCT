...
from models.vision_transformer import VisionTransformer, _load_weights 
...
def get_args():
    ...
    parser = argparse.ArgumentParser(description='VCT exps')

    # path
    parser.add_argument('--data', default='./imagenet', help='path to dataset')
    ...
    # Exp Settings
    parser.add_argument('--method', default='vct', type=str, help='no_adapt, tent, eata, sar, vct')
    ...
    parser.add_argument('--pretrained_model_path', default = './models/B_16-i21k-300ep-lr_0.001-aug_medium1-wd_0.1-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.01-res_224.npz', help='pretrained model path')
    parser.add_argument('--cls_token_lr', default=0.005, type=float, help='lerning rate of the class token')
    parser.add_argument('--instance_token_lr', default=0.01, type=float, help='lerning rate of the instance token')
    parser.add_argument('--token_reset', default=True, type=bool, help='token_reset')
...
if __name__ == '__main__':
    ...
    if args.exp_type == 'mix_shifts':
        ...
        for cpt in common_corruptions:
            ...
            if args.method in ['tent', 'no_adapt', 'eata', 'sar', 'vct']:
    ...
    for corrupt in common_corruptions:
        ...
        if args.method in ['tent', 'eata', 'sar', 'vct', 'no_adapt']:
        ...
        if args.method in ['tent', 'eata', 'sar', 'vct', 'no_adapt']:
            if args.model == "resnet50_gn_timm":
                ...
                net = VisionTransformer(batch_size = args.test_batch_size)
                _load_weights(net, checkpoint_path = args.pretrained_model_path)
                args.lr = (0.001 / 64) * bs
        ...
        if args.exp_type == 'bs1' and (args.method == 'sar' or args.method == 'vct'):
            args.lr = 2 * args.lr
            logger.info("double lr for sar/vct under bs=1")
        ...
        if args.method == "tent":
            ...
            logger.info(f"acc1s are {acc1s}")
            logger.info(f"acc5s are {acc5s}")
        elif args.method == 'vct':
            net = vct.configure_model(net)
            params, param_names = vct.collect_params(net, args)
            # logger.info(f"model for adaptation: %s", net)
            logger.info(param_names)

            base_optimizer = torch.optim.SGD
            optimizer = SAM(params, base_optimizer, momentum=0.9)
            adapt_model = vct.VCT(net, optimizer, margin_e0=args.sar_margin_e0)
            ...
            logger.info(f"Result under {args.corruption}. The adaptation accuracy of VCT is top1: {acc1:.5f} and top5: {acc5:.5f}")
