...
from models.helpers import build_model_with_cfg, resolve_pretrained_cfg, named_apply, adapt_input_conv, checkpoint_seq
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.instance_token = nn.Parameter(torch.zeros(batch_size, 1, embed_dim))  # initialize the instance token
    ...
    def init_weights(self, mode=''):
        ...
        if self.cls_token is not None:
            ...
            nn.init.constant_(self.instance_token, 0.)
    ...
    def _pos_embed(self, x):
        if self.no_embed_class:
            ...
            x = x + self.pos_embed
            if self.cls_token is not None:
                x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x) + self.instance_token[:x.shape[0]], dim=1)
        else:
            # original timm, JAX, and deit vit impl
            # pos_embed has entry for class token, concat then add
            if self.cls_token is not None:
                x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1) + self.instance_token[:x.shape[0]], x), dim=1)
            x = x + self.pos_embed
